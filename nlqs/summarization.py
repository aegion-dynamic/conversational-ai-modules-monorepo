...
def summarize(
    ...
    # REFACTOR: Move the prompt creation to a separate function
    ...
    output_parser = StrOutputParser() # TODO: Use JsonOutputParser
    chain = intent_classification_prompt | llm | output_parser

    summarized_input_intent_raw = str(chain.invoke({"user_input": user_input}))

    logger.debug(f"summarized_input_intent: {summarized_input_intent_raw}")
    ...
    # REFACTOR: Move out this prompt creation to a separate function
    ...
    try:
        ...
        logger.debug(f"summarized_input_dict: {summarized_input_dict}")
        ...
        # If parsing fails, return an empty SummarizedInput
        # REFACTOR: .exception over .error for logging exceptions
    ...
    def _add_column(column_name: str, column_type: ColumnType, description: str):
        nonlocal numerical_data, categorical_data, descriptive_data, identifier_data
    ...
    for column_name, description in summarized_input_dict.get("quantitative_data", {}).items():
        if column_name not in column_descriptions_dictionary: # Because the column is generated by the LLM, it may not be present in the data
            logger.info(f"Column '{column_name}' not found in column descriptions. Finding closest match...")
            ...
            logger.info(f"Closest column name found: {closest_column_name} of type {column_type}")
            ...
            # Add the column to the corresponding dictionary
            _add_column(closest_column_name, column_type, description)
    ...
    process_qualitative_data(
        summarized_input_dict,
        column_descriptions_dictionary,
        vectordb,
        numerical_data,
        categorical_data,
        descriptive_data,
...
def process_qualitative_data(
    summarized_input_dict: Dict,
    column_descriptions_dictionary: Dict[str, str],
    vectordb: VectorDBDriver,
    numerical_data: Dict[str, str],
    categorical_data: Dict[str, str],
    ...
    """Processes qualitative data from the summarized input and populates the appropriate data dictionaries.

    Args:
        summarized_input_dict (Dict): The summarized input dictionary containing qualitative_data.
        column_descriptions_dictionary (Dict[str, str]): Available column descriptions.
        vectordb (VectorDBDriver): The vector database driver for column lookups.
        numerical_data (Dict[str, str]): Dictionary to populate with numerical column data.
        categorical_data (Dict[str, str]): Dictionary to populate with categorical column data.
        descriptive_data (Dict[str, str]): Dictionary to populate with descriptive column data.
        identifier_data (Dict[str, str]): Dictionary to populate with identifier column data.
    """
    def _add_column(column_name: str, column_type: ColumnType, description: str):
        # Add the column to the corresponding dictionary
        if column_type == ColumnType.NUMERICAL:
    ...
    for column_name, description in summarized_input_dict.get("qualitative_data", {}).items():
        if column_name not in column_descriptions_dictionary.keys():
            ...
            if closest_column_name not in column_descriptions_dictionary: # COMMENT: why wouldn't the closest column name be in column_descriptions_dictionary?
                raise ValueError(f"Closest column name '{closest_column_name}' not found in chroma columns collection.")
            ...
            column_type = vectordb.get_column_type(column_name, DEFAULT_TABLE_NAME, DEFAULT_DB_NAME)
            _add_column(column_name, column_type, description)

...
from tree_of_thoughts_executor import ToTExecutorInputs, TreeOfThoughtsExecutor
...
class CannabisRecommendationSystem:
    ...
    def chat(self, user_input: str, chat_history: List[Tuple[str, str]]) -> Dict[str, Any]:
...
def main():
    system = CannabisRecommendationSystem()
    print("Welcome to the Cannabis Recommendation System!")
    print("How can I assist you in finding the right cannabis product today?")
    chat_history = []
    while True:
        user_input = input("You: ").strip()
        if user_input.lower() in ["exit", "quit", "bye"]:
            print("\nThank you for using the Cannabis Recommendation System.")
            print("Here's a summary of our conversation:")
            print_session_summary(system.session_data)
            print("\nGoodbye!")
            break

        response = system.chat(user_input, chat_history)
        if not response:
        ...
        if response["response"]["type"] == "recommendation":
            recommendation_result = system.format_recommendation(response)
        ...
        print(f"Assistant: {result.description}")
        if result.options:
            print("Options:")
        ...
        print("\nRAG Analysis:")
        print(json.dumps(system.session_data["rag_analysis"], indent=4))

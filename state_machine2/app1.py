"""
Medical Cannabis Recommendation System

This module implements a conversational AI system for providing medical cannabis recommendations.
It uses the OpenAI API to generate responses and guide users through a structured conversation
about their medical needs and potential cannabis treatments.
...
Note: This version uses a state machine approach to guide the conversation.
"""
...
class CannabisRecommendationSystem:
    """
    Main class for handling the cannabis recommendation system logic.

    This class manages the conversation state, processes user input, and generates
    appropriate responses using the OpenAI API.
    ...
    def __init__(self, api_key: str):
        ...
        self.state_requirements = {
            State.INITIAL_INQUIRY: {
                ...
                "goal": "Determine if the user has a medical query related to cannabis."
            },
            State.MEDICAL_ASSESSMENT: {
                "required_info": ["main_symptom", "symptom_duration", "previous_treatments"],
                "goal": "Gather essential information about the user's medical condition."
            },
            State.RECOMMENDATION: {
                "required_info": ["suitable_products", "usage_guidelines", "precautions"],
                "goal": "Provide a tailored cannabis product recommendation based on the user's condition."
            },
            State.CONCLUSION: {
                "required_info": ["user_understanding", "next_steps"],
                "goal": "Ensure the user understands the recommendation and knows the next steps."
            }
        }
    ...
    def get_llm_response(self, user_input: str, state_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        3. If necessary, ask a single, focused follow-up question to gather missing information.
        4. Provide 2-3 sample options for the user to choose from in their response.
        5. Determine if the current state's goal has been sufficiently met to transition to the next state.
...
class CannabisRecommendationApp:
    """
    Application class to run the Cannabis Recommendation System.
    ...
    def run(self):
        ...
        while True:
            ...
            if self.system.state == State.CONCLUSION:
                break

def main():
    """
    Main function to run the Cannabis Recommendation App.
    """
